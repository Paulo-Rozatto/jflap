...
/**
 * The useless states detector object can be used to find all states in an automaton that either are
 * not reachable from the initial state, or that cannot reach a final state. This does only naive
 * checking for the existence of edges. It cannot be used, for example, to solve the halting problem
...
public class UselessStatesDetector {
   ...
   * @return a set containing all states in the automaton that are unreachable from the initial
   *     state or cannot lead to a final state
