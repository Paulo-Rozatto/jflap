...
/**
 * The fsa lambda transition checker can be used to determine if a finite state automaton's
...
public class FSALambdaTransitionChecker extends LambdaTransitionChecker {
  ...
  /**
   * Returns true if <CODE>transition</CODE> is a lambda transition (i.e. it's label is the lambda
   * string).
   *
   * @param transition the transition
   * @return true if <CODE>transition</CODE> is a lambda transition (i.e. it's label is the lambda
   *     string).
   */
