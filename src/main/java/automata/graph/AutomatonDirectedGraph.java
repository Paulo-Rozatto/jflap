...
/**
 * Constructs a graph of an automaton, but does not, for each edge, classify the object to which an
 * edge points as adjacent to the object from which the edge points. It only classifies a "from"
 * object as adjacent to a "to" object. The default <code>degree()</code> method for this graph will
 * return the number of all edges leading from a vertex, including those pointing to itself.
 *
 * @author Chris Morgan
 */
public class AutomatonDirectedGraph extends AutomatonGraph {
   ...
   * @param excludeSameVertexEdges boolean that if <i>true</i> will exclude edges leading from and
   *     to the same vertex from consideration.
   */
