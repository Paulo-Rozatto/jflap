...
/**
 * A <code>MealyConfiguration</code> object is a <code>Configuration</code> object with an
 * additional field for the input string and the output string. The output is saved as a separate
 * field that is passed in through the constructor. All additional output appended to the output
 * passed in. A <code>MealyConfiguration</code> serves both the {@link automata.mealy.MealyMachine}
 * and the {@link automata.mealy.MooreMachine}
 *
 * @author Jinghui Lim
 */
public class MealyConfiguration extends Configuration {
  ...
  /**
   * Returns the output produced by previous ancestor configurations. This is the only way to obtain
   * the output produced by a Mealy machine, as <code>Configuration.isAccept()</code> returns a
   * <code>boolean</code>.
   *
  ...
  /**
   * Accepts if there is no more unprocessed input (all of the input has been process and output
   * generated).
   *
   * @return <code>true</code> if all input has been processed, <code>false</code> otherwise.
   */
  ...
  /**
   * This function checks if two configurations are equal. Two configurations are equal if they have
   * the same output and unprocessed input and satisfy the <code>.equals()</code> test of the base
