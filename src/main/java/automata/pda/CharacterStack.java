...
public class CharacterStack implements Serializable {
  ...
  /**
   * Pushes a string onto a stack. The first character in the string is the last pushed on the
   * stack.
   *
  ...
  /**
   * Pops a character from this stack. This will remove that character from the stack.
   *
   * @return the top character in the stack, or 0 if there is no character in the stack
   */
  public char pop() {
  ...
  /**
   * Pops a number of characters off the stack, and returns the result as a string. The first
   * character in the string is the first character popped off the stack.
   *
   * @param number the number of elements to pop off the stack
   * @return a string of length <CODE>number</CODE> or <CODE>null</CODE> if there are not <CODE>
   *     number</CODE> characters left on the stack
   */
  ...
  /**
   * Predictably, two character stacks are equal if they have the same characters in the stack in
   * the same order, etc.
   *
  ...
  public boolean equals(Object stack) {
    try {
      return ((CharacterStack) stack).buffer.toString().contentEquals(buffer);
