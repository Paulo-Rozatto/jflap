...
/**
 * A <CODE>PSAConfiguration</CODE> object is a <CODE>Configuration</CODE> object with additional
 * fields for the input string and the stack contents. The current state of the automaton, the stack
 * contents, and the unprocessed input are the only necessary data for the simulation of a PDA.
 *
...
public class PDAConfiguration extends Configuration {
   ...
   *
   * @param input the string that will represent the unprocessed input of the PDA.
   */
  ...
  /**
   * Returns a string representation of this object. This is the same as the string representation
   * for a regular configuration object, with the additional fields tacked on.
   *
  ...
  /**
   * Returns <CODE>true</CODE> if this configuration is an accepting configuration, which in this
   * case means that there is no more input and our state is an accept state.
   *
   * @return <CODE>true</CODE> if this configuration is accepting, <CODE>false</CODE> otherwise
   */
  public boolean isAccept() {
    if (myAcceptance == FINAL_STATE) {
      ...
      CharacterStack stack = this.getStack();
        return this.getUnprocessedInput() == "" && stack.height() == 0;
    }
    return false;
  }

  /**
   * Determines whether this configuration equals another configuration. Two PDA configurations are
   * equal if they have the same stack, and if they satisfy the <CODE>Configuration.equals()</CODE>
   * method.
   *
   * @param configuration the configuration to check for equality
   * @return <CODE>true</CODE> if the configuration is equal to this one, <CODE>false</CODE> if it
   *     is not
