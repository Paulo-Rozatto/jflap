...
/**
 * The pda lambda transition checker can be used to determine if a pushdown automaton's transition
...
public class PDALambdaTransitionChecker extends LambdaTransitionChecker {
  ...
  /**
   * Returns true if <CODE>transition</CODE> is a lambda transition (i.e. all three of its fields
   * are the lambda string).
   *
   * @param transition the transition
   * @return true if <CODE>transition</CODE> is a lambda transition (i.e. all three of its fields
   *     are the lambda string).
   */
  public boolean isLambdaTransition(Transition transition) {
    ...
    String toPush = trans.getStringToPush();
      return input.equals(LAMBDA) && toPop.equals(LAMBDA) && toPush.equals(LAMBDA);
  }
}
