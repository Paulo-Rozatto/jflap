...
/**
 * The PDA nondeterminism detector object can be used to find all the nondeterministic states in a
 * pushdown automaton (i.e. all states with equivalent outward transitions).
 *
...
public class PDANondeterminismDetector extends NondeterminismDetector {
  ...
  /**
   * Returns true if either of the two strings is a prefix of the other string.
   *
   * @return true if either of the two strings is a prefix of the other string.
   */
  ...
  /**
   * Returns true if the transitions are identical (i.e. the first 2 components of the labels are
   * equivalent) or if the transitions introduce nondeterminism (e.g. the labels of one transition
   * could be a prefix of the label of the other transition.)
   *
  ...
  public boolean areNondeterministic(Transition t1, Transition t2) {
    ...
    return arePrefixesOfEachOther(input1, input2) && arePrefixesOfEachOther(toPop1, toPop2);
  }
}
