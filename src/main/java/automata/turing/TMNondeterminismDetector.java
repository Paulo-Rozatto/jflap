...
/**
 * The TTM nondeterminism detector object can be used to find all the nondeterministic states in a
 * Turing machine (i.e. all states with transitions that read the same symbols on each tape).
 *
...
public class TMNondeterminismDetector extends NondeterminismDetector {
  ...
  /**
   * Returns true if the transitions introduce nondeterminism (e.g. the input to read from tapes one
   * and two portions of the transition labels are identical).
   *
      ...
      if (!read1.equals(read2)
        ...
        if (read1.startsWith("!")
          ...
          for (int k = 0; k < read2.length(); k++) {
            ...
            if (k == (read2.length() - 1)
                && !foundMatch) { // iterated through all letters and no matches
              return false;
            }
          }
        }
