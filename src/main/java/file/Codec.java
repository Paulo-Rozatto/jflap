...
public abstract class Codec extends FileFilter implements Encoder, Decoder {
  /**
   * Whether the given file is accepted by the codec. This method is implemented to, by default,
   * always return true.
   *
  ...
  /**
   * Given a proposed filename, returns a new suggested filename. This method is implemented in this
   * abstract class to always return the original filename, so subclasses that wish more desirable
   * functionality should override this method to, say, add a suffix for example.
   *
  ...
  /**
   * Returns an instance of a corresponding encoder. In many cases the returned will be <CODE>this
   * </CODE>.
   *
   * @return an encoder that encodes in the same format this decodes in, or <CODE>null</CODE> if
  ...
  /**
   * Returns the root of a filename that supposedly belongs to this codec. The default
   * implementation assumes that if a modification will happen, it will be some suffix with a
   * period.
   *
